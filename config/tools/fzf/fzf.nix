...
in {
  ...
  vim.lazy.plugins.fzf-lua = {
    ...
    keys = [
      ...
      (mkKeymap "n" "<leader>/" "<CMD>lua require('fzf-lua').live_grep({ cwd = require('root').get() })<CR>" {desc = "Grep (root directory)";})
      (mkKeymap "n" "<leader>:" "<CMD>FzfLua command_history<CR>" {desc = "Command History";})
      (mkKeymap "n" "<leader><space>" "<CMD>lua require('fzf-lua').files({ cwd = require('root').get() })<CR>" {desc = "Files (root directory)";})
      # Files
      (mkKeymap "n" "<leader>ff" "<CMD>lua require('fzf-lua').files({ cwd = require('root').get() })<CR>" {desc = "Files (root directory)";})
      ...
      (mkKeymap "n" "<leader>fR" "<CMD>lua require('fzf-lua').oldfiles({ cwd = vim.fn.expand('%:p:h') })<CR>" {desc = "Find Recent Files (buffer directory)";})
      # Git
      ...
      (mkKeymap "n" "<leader>sg" "<CMD>lua require('fzf-lua').live_grep({ cwd = require('root').get() })<CR>" {desc = "Search Grep (root directory)";})
      (mkKeymap "n" "<leader>sG" "<CMD>lua require('fzf-lua').live_grep({ cwd = vim.fn.expand('%:p:h') })<CR>" {desc = "Search Grep (buffer directory)";})
      ...
      (mkKeymap "n" "<leader>sw" "<CMD>lua require('fzf-lua').grep_cword({ cwd = require('root').get() })<CR>" {desc = "Search Word (root directory)";})
      (mkKeymap "n" "<leader>sW" "<CMD>lua require('fzf-lua').grep_cword({ cwd = vim.fn.expand('%:p:h') })<CR>" {desc = "Search Word (buffer directory)";})
      (mkKeymap "v" "<leader>sw" "<CMD>lua require('fzf-lua').grep_visual({ cwd = require('root').get() })<CR>" {desc = "Search Visual Selection (root directory)";})
